...
st.markdown("""
    ...
    <h1 style="font-size: 1.0rem; font-weight: 700; color: #ffffff; text-align: center;">version 2.0</h1>
...
def display_player_stats_page():
    ...
    try:
        ...
        # 모든 선수 목록 가져오기
        for player in sorted(player_stats, key=lambda x: x['이름']):  # 이름 순 정렬
            name = player['이름']
...
# 사용 예시
if st.button("대회 삭제"):
    if confirm_dialog("정말 이 대회를 삭제하시겠습니까?", "delete_tournament_confirm"):
        # 확인된 경우 삭제 실행
...
def display_admin_page():
    ...
    # 탭 추가
    tabs = st.tabs(["대회 관리", "선수 관리", "데이터베이스 관리"])
   ...
   # 데이터베이스 초기화
    ...
    with st.container():
        ...
        selected_page = st.radio(
            "", 
            ["스코어 입력", "선수별 기록", "관리자 도구"], 
            horizontal=True, 
            label_visibility="collapsed",
            index=0 if current_page == "스코어 입력" else 
                1 if current_page == "선수별 기록" else 2,
